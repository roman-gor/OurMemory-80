...
- **LiteRT 1.4.2, а не 2.x.** В LiteRT 2.x у `litert` и `litert-api` одинаковый namespace `com.google.ai.edge.litert`, и AGP 9 валит `processDebugMainManifest`. В 1.4.2 namespace разные, а `org.tensorflow.lite.Interpreter` тот же.
- **Модель весит 12,3 МБ, а не 3–5 МБ.** MobileNet V2 1.4 содержит ~6 млн весов, при float16 это ~12 МБ. Выход float16 совпадает с исходной float32-моделью до 0,02. Вместе с `libtensorflowlite_jni.so` под четыре ABI APK вырос примерно на 32 МБ.
- **Скрипт конвертирует `saved_model`, а не Keras `.h5`.** Старый `.h5` не читается Keras 3, а `TFLiteConverter.from_saved_model` в TensorFlow 2.15 работает без Keras.
- **Ложные срабатывания в словаре.** `блят` находился в «употреблять», «оскорблять», «истреблять», а `ебал` / `ебан` / `ебат` в «колебание», «небанальный», «погребальный», «выгребать». Поэтому `блят` убран из корней, `блять` перенесён в точные слова, а в безопасные фрагменты добавлены `греб`, `колеб`, `небан`, `небал`, `ребал`. В точные слова также добавлены формы `ебу`, `еби`, `ебет`, `ебешь`, `ебут`, `ебись`.
...
- **Подсвечивается именно то поле, где найден мат.** Вместо одного `hasProfanity` в состоянии два флага: `hasTextProfanity` и `hasContactProfanity`. Иначе при мате в контакте краснело бы поле текста.
- **Повторно выбранное фото не дублируется.** Пока фото проверяется, его можно выбрать ещё раз. Поэтому `withCheckedPhoto` добавляет URI, только если его ещё нет в списке, иначе `LazyRow` упал бы на повторяющемся ключе.
- **Существующие тесты** `SubmissionViewModelTest` и `FeedbackViewModelTest` получили `FakeContentCheckRepository`, иначе они не компилировались. Новых тестов нет.
